...
| `0x30C0-0x3521` | 1122 | Current box cache | synchronized duplicate/cache data | derived from target permanent storage and selected box | Save Genie layout exposes current-box cache as distinct from permanent boxes. | Very high; stale cache can disagree with permanent boxes. | Reparse current-box cache and compare against permanent storage. |
