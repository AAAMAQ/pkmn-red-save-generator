...
| `0x0598-0x1857` | 4800 | Hall of Fame records | overwritten from target semantic input | target `decoded.hallOfFame` | Save Genie layout and schema expose Hall of Fame directly. | Medium; entry ordering and padding must stay coherent. | Reparse and semantic comparison. |
...
| `0x2598-0x29F2` | 1115 | Core Bank 1 semantic region before event flags split | mixed: overwritten target fields plus inherited map-runtime fields | target core fields where explicitly owned; dummy template for load-sensitive map-runtime bytes | Save Genie layout places these fields in checksum-covered Bank 1; emulator incident proved parser-visible location bytes are not enough. | Very high; map-runtime incoherence caused immediate load-time corruption. | Reparse, semantic comparison, byte-provenance report, and emulator base-load gate. |
| `0x29F3-0x2CF3` | 769 | Event/story/world-state region | overwritten from target semantic input for named event/script fields; inherited for unowned runtime-adjacent values | target `decoded.events`, `storyProgress`, `trainerBattles`, `staticBattles`, `visitedTowns`, `hiddenItems`, `hiddenCoins`, `scripts`, `missableObjects`, and selected runtime bits only | Save Genie docs and schema expose these as decoded semantic regions, but not every runtime-adjacent value is safe to regenerate. | High; contradictions can create impossible states. | Semantic comparison and emulator safety tests. |
| `0x2CF4-0x300B` | 792 | Daycare and adjacent persistent gameplay state | overwritten from target semantic input | target `decoded.daycare` | Save Genie layout exposes daycare offsets and tests validate decode. | High if inherited or half-written. | Reparse and focused daycare tests. |
...
| `0x30C0-0x3521` | 1122 | Current working box/cache | independent player-visible storage state | written from target `storage.currentBoxCache`; not derived from the selected permanent box | Save Genie and emulator evidence show the Bank 1 working box may legitimately be newer than its permanent-bank copy until a box switch. | Very high; canonicalizing it from permanent storage can erase visible Pokemon. | Validate cache structure independently, classify any selected permanent/cache divergence, and test box switching in the emulator. |
...
| `0x4000-0x5A4B` | 6732 | Permanent PC Boxes 1-6 payload | overwritten from target semantic input | target `decoded.pcStorage` | Save Genie layout and schema expose all 12 boxes. | Critical; malformed storage can corrupt the save or break PC interaction. | Reparse box contents and checksum validation. |
...
| `0x6000-0x7A4B` | 6732 | Permanent PC Boxes 7-12 payload | overwritten from target semantic input | target `decoded.pcStorage` | Save Genie layout and schema expose all 12 boxes. | Critical; malformed storage can corrupt the save or break PC interaction. | Reparse box contents and checksum validation. |
...
- The current dummy's invalid permanent box contents were the highest-risk inherited areas in early milestones. Generated outputs now replace them completely.
